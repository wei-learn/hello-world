...
v4 <- v3 %>%
       mutate(across(.cols = ends_with("Rate"), ~percent_format(accuracy = 0.1)(.x))) %>%
...
chr <- cmb %>%
      ...
      summarise(Request = sum(`#_request_submitted`, na.rm=FALSE),                                                     Hit=sum(`#_requests_with_one_or_more_Records_Found`),                                                  Req_w_Rls=sum(`#_requests_with_one_or_more_Records_Released`),
                Release = sum(`Records_Released`)) %>%
      mutate(Hit_Rate=Hit/Request, Release_Rate=Req_w_Rls/Request, Avg_Rls_Req = Release/Req_w_Rls) %>%
      select(vendor_name, request_date_by_week, Request, Hit, Release, Req_w_Rls, Hit_Rate, Release_Rate, Avg_Rls_Req) %>%
