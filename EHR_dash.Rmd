...
ehrin <- "D:/Projects/EHR/Weekly/weekly_10072020.xlsx"
...
<h2 style="color:grey;text-align:center;"> Week of `r mwk` to `r mwk+7` </h2>
...
### Vendor Request and Release Rate
...
cmbn2 <- Vendor %>%
      ...
      group_by(vendor_name, request_date_by_week) %>%
      summarise(Request = sum(num_request_submitted, na.rm=FALSE),                                                     Req_w_Rls=sum(num_requests_with_one_or_more_records_released),
                Release = sum(num_records_released)) %>%
      mutate(Release_Rate=Req_w_Rls/Request, Avg_Rls_Req = Release/Req_w_Rls) %>%
      select(vendor_name, request_date_by_week, Request, Release, Req_w_Rls, Release_Rate, Avg_Rls_Req) %>%
      arrange(desc(request_date_by_week), vendor_name)

#transpose, fill missing with 0
v<- pivot_wider(cmbn2, names_from = vendor_name, names_sort = TRUE, names_glue = "{vendor_name}_{.value}", values_from=c(Request, Release, Req_w_Rls, Release_Rate, Avg_Rls_Req), values_fill=0)
...
colttl <- cbind(data.frame(request_date_by_week=as.Date(0)), data.frame(summarise(v2, across(ends_with(c("Request", "Rls", "Release")), sum))))

library(data.table)
v3<-rbind(v2[, colnames(v2) %ilike% "^request"| colnames(v2) %ilike% "Request$" | colnames(v2) %ilike% "Rls$" | colnames(v2) %ilike% "Release$"], colttl)


v4<- left_join(v3, v2[, !(colnames(v2) %ilike% "Request$" | colnames(v2) %ilike% "Rls$" | colnames(v2) %ilike% "Release$")], by="request_date_by_week")
...
<center> _Search:	An operation a vendor conducts to look for one or multiple matches in their data sources_</center> 
...
vdr <- cmbn2 %>%
      filter(request_date_by_week==mwk2)
...
<h2 style="color:grey;text-align:center;"> Week of `r mwk` </h2>
